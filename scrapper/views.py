...
def scrape_ipo_data(html_content):
    ...
    for table in table_data:
        rows = table.find_all("tr")[
            1:8
        ]  # Restrict to the top 7 rows (skip the header row)
        ...
        for row in rows:
            ...
            ipo_data = {
                ...
                "IPOGMP": (
                    html.unescape(cells[3].get_text().strip())
                    if len(cells) > 3
                    else None
                ),
                "IPOPrice": (
                    html.unescape(cells[4].get_text().strip())
                    if len(cells) > 4
                    else None
                ),
...
def fetch_stock_data(request, ticker_symbol):
    ...
    data = yf.download(
        ticker_symbol, start=start_date, end=datetime.now().strftime("%Y-%m-%d")
    )["Close"]
    data = data.astype(int)

    result = []
    for date, close in data.items():
        result.append({"date": date.strftime("%Y-%m-%d"), "Nifty": int(close)})
